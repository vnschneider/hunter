...
| CI (lint/typecheck/build web+worker)                                          | Por fazer       |
...
| Item                                                                          | Estado                                    |
| ----------------------------------------------------------------------------- | ----------------------------------------- |
| Upload CV → Storage + metadados em `cvs`                                      | Feito (Settings)                          |
| Assistente "CV → estratégia" (LLM barato + revisão humana)                    | Feito (Gemini/OpenAI/Groq + fallback)     |
| API/UX para enfileirar e listar hunts                                         | Feito (`/dashboard/hunts` + `/api/hunts`) |
| Worker MVP que consome `jobs` e atualiza `hunts/hunt_events`                  | Feito (`apps/web/scripts/worker.mjs`)     |
| MCP próprio (server + conectores GitHub/Greenhouse/Lever + score/rank)        | Feito (v1 em `apps/web/scripts/mcp`)      |
| Integração MCP (worker + conectores + score/rank) gravando hunts/applications | Feito                                     |
...
| Item                                     | Estado                             |
| ---------------------------------------- | ---------------------------------- |
| SSE (ou polling) para eventos da hunt    | Feito (polling no painel de hunts) |
| Relatórios por período (queries + UI)    | Por fazer                          |
| `Dockerfile` / compose para web + worker | Feito                              |
...
**Estratégias, worker MCP, Docker e listagem de candidaturas estão feitos (com `DATABASE_URL`). Falta fechar SSE/relatórios e hardening de operação.**
